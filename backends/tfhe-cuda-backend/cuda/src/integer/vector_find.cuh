...
// gridDim.x == current_chunk, gridDim.y == num_blocks.
template <typename T>
__global__ void
...
__host__ void host_n_ary_blockwise_addition(
  ...
  n_ary_blockwise_addition<T><<<grid, thds, 0, stream>>>(
      (T *)output->ptr, (const T *)input->ptr, row_offset, row_count,
      num_columns, lwe_size, num_entries);
  check_cuda_error(cudaGetLastError());
...
__host__ void host_compute_eq_selectors_ct_vs_clears(
  ...
  host_and_reduce_selector_matrix<Torus>(
      ...
      mem_ptr->luts_eq.data(), mem_ptr->num_luts_needed, num_possible_values,
...
__host__ void host_compute_eq_selectors_cts_vs_ct(
  ...
  for (uint32_t base = 0; base < num_inputs; base += chunk_size) {
    ...
    host_and_reduce_selector_matrix<Torus>(
        ...
        mem_ptr->luts_eq.data(), mem_ptr->num_luts_needed, current_chunk,
        num_blocks, max_degree, message_modulus, carry_modulus, bsks, ksks);
...
__host__ void host_unchecked_match_value(
  ...
  if (!mem_ptr->max_output_is_zero) {
    host_create_possible_results<Torus>(
        streams, mem_ptr->possible_results_list.data(),
        &mem_ptr->packed_selectors_ct, mem_ptr->num_matches, h_match_outputs,
  ...
  host_aggregate_one_hot_vector<Torus>(
      streams, lwe_array_out_result, mem_ptr->possible_results_list.data(),
...
host_unchecked_contains(CudaStreams streams, CudaRadixCiphertextFFI *output,
  ...
  host_compute_eq_selectors_cts_vs_ct<Torus>(
      streams, mem_ptr->unpacked_selectors.data(), inputs, value, num_inputs,
...
__host__ void host_unchecked_contains_clear(
  ...
  host_compute_eq_selectors_cts_vs_ct<Torus>(
      streams, mem_ptr->unpacked_selectors.data(), inputs,
...
__host__ void host_compute_final_index_from_selectors(
  ...
  host_create_possible_results<Torus>(
      streams, mem_ptr->possible_results_ct_list.data(), packed_selectors,
      num_inputs, mem_ptr->h_indices, packed_len, mem_ptr->possible_results_buf,
      bsks, ksks);

  host_aggregate_one_hot_vector<Torus>(
      streams, index_ct, mem_ptr->possible_results_ct_list.data(), num_inputs,
...
__host__ void host_unchecked_index_in_clears(
  ...
  host_create_possible_results<Torus>(
      streams, mem_ptr->final_index_buf->possible_results_ct_list.data(),
  ...
  host_aggregate_one_hot_vector<Torus>(
      streams, index_ct,
      mem_ptr->final_index_buf->possible_results_ct_list.data(), num_clears,
      packed_len, mem_ptr->final_index_buf->aggregate_buf, bsks, ksks);
...
__host__ void host_unchecked_first_index_in_clears(
  ...
  host_create_possible_results<Torus>(
      streams, mem_ptr->possible_results_ct_list.data(),
      &mem_ptr->packed_selectors, num_unique, h_unique_indices, packed_len,
      mem_ptr->possible_results_buf, bsks, ksks);

  host_aggregate_one_hot_vector<Torus>(
      streams, index_ct, mem_ptr->possible_results_ct_list.data(), num_unique,
      packed_len, mem_ptr->aggregate_buf, bsks, ksks);
...
__host__ void host_unchecked_first_index_of_clear(
  ...
  host_create_possible_results<Torus>(
      streams, mem_ptr->possible_results_ct_list.data(),
      &mem_ptr->packed_selectors, num_inputs,
      (const uint64_t *)mem_ptr->h_indices, packed_len,
      mem_ptr->possible_results_buf, bsks, ksks);
...
__host__ void host_unchecked_index_of(
  ...
  host_compute_eq_selectors_cts_vs_ct<Torus>(
      streams, mem_ptr->final_index_buf->unpacked_selectors.data(), inputs,
      value, num_inputs, num_blocks, mem_ptr->eq_selectors_buf, bsks, ksks);
  ...
  host_aggregate_one_hot_vector<Torus>(
      streams, index_ct,
      mem_ptr->final_index_buf->possible_results_ct_list.data(), num_inputs,
      packed_len, mem_ptr->final_index_buf->aggregate_buf, bsks, ksks);
